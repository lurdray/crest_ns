...
def ResultView(request, domain_name):
	...
	if request.method == "POST":
		...
		name_address = requests.get("https://api.iotexchartapp.com/cns/get-address/%s/" % (domain_name)).json()["address"]
		...
		name_price = requests.get("https://api.iotexchartapp.com/cns/get-price/%s/" % (domain_name)).json()["price"]
...
def BuyView(request, domain_name):
	...
	if request.method == "POST":
		...
		context = {"name_price": name_price, "domain_name": domain_name,}
...
def TransferView(request):
	if request.method == "POST":
		...
		return HttpResponseRedirect(reverse("main:transfer2", args=[domain_name,]))
